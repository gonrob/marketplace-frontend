...
export default function Admin() {
  ...
  const tabStyle = (t) => ({
    ...
    border:'1.5px solid #4B6CB7',borderRadius:8,cursor:'pointer'
  });
                ...
                <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:'flex',gap:12,marginTop:8}}>
                      <div style={{fontSize:13,color:'#4B6CB7',fontWeight:600}}>USD {(s.ganancias||0).toFixed(2)} ganados</div>
                      <div style={{fontSize:13,color:'#888'}}>{s.totalContactos||0} contactos</div>
                      <div style={{fontSize:13,color:'#4B6CB7'}}>USD {s.precio||10}/contacto</div>
                    </div>
                    <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
                      <a href={`mailto:${s.email}`}>
                        <button style={{width:'auto',padding:'6px 12px',fontSize:12,background:'#4B6CB7',color:'white',border:'none',borderRadius:6}}>📧 Email</button>
                      </a>
                      {s.telefono && (
          ...
          <div>
            {pagos.length === 0 ? (
              <div className="card" style={{textAlign:'center',color:'#888'}}>
                ...
                <p style={{fontSize:13}}>Ve a <a href="https://dashboard.stripe.com/payments" target="_blank" rel="noopener noreferrer" style={{color:'#4B6CB7'}}>dashboard.stripe.com/payments</a> para ver todos los pagos.</p>
              </div>
            ) : (
              pagos.map((p, i) => (
