...
export default function HostOnboarding({ onComplete }) {
  ...
  const toggle = (id) => {
    setSeleccionados(p => { const n = {...p}; if (n[id]) delete n[id]; else n[id] = true; return n; });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const g of galeria) {
        ...
        if (ur.ok) { const d = await ur.json(); galeriaUrls.push(d.url); }
      }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '24px 16px 48px' }}>
      <div style={{ background: '#fff', borderRadius: 20, maxWidth: 620, width: '100%', padding: '28px 24px', boxShadow: '0 8px 48px rgba(0,0,0,0.2)' }}>
          ...
          <div style={{ background: '#f0f4ff', borderRadius: 14, padding: 18, marginBottom: 16, border: '1.5px solid #4B6CB7' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              <div>
                ...
                <input type="number" placeholder="ej: 25" value={precio} onChange={e => setPrecio(e.target.value)} style={inp(false)} />
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginBottom: 4 }}>⏱️ Duración promedio</label>
                <select value={duracion} onChange={e => setDuracion(e.target.value)} style={inp(false)}>
            ...
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginBottom: 4 }}>📝 Descripción</label>
              <textarea placeholder="Ej: Soy guía local, conozco todos los rincones de Buenos Aires..." value={descripcion} onChange={e => setDescripcion(e.target.value)} rows={3} style={{ ...inp(false), resize: 'vertical' }} />
            </div>
            <div>
        ...
        {/* SERVICIO CUSTOM */}
        <div style={{ background: '#fffbea', borderRadius: 14, padding: 18, marginBottom: 16, border: '1.5px solid #fcd34d' }}>
          <p style={{ fontWeight: 700, fontSize: 14, margin: '0 0 4px', color: '#92400e' }}>✨ ¿Tenés algo único para ofrecer?</p>
          <p style={{ color: '#888', fontSize: 12, margin: '0 0 12px' }}>Escribilo con tus palabras — se destacará en tu perfil</p>
        ...
        {/* GALERIA */}
        <div style={{ background: '#f8faff', borderRadius: 14, padding: 18, marginBottom: 20, border: '1.5px solid #e5e7eb' }}>
          <p style={{ fontWeight: 700, fontSize: 14, margin: '0 0 4px', color: '#222' }}>🖼️ Galería de fotos (opcional)</p>
          <p style={{ color: '#888', fontSize: 12, margin: '0 0 14px' }}>Hasta 5 fotos de tus experiencias</p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {galeria.map((g, i) => (
              <div key={i} style={{ position: 'relative', width: 80, height: 80 }}>
                ...
                <button onClick={() => setGaleria(p => p.filter((_, j) => j !== i))} style={{ position: 'absolute', top: -6, right: -6, background: '#ef4444', color: '#fff', border: 'none', borderRadius: '50%', width: 20, height: 20, fontSize: 11, cursor: 'pointer' }}>✕</button>
              </div>
            ))}
            {galeria.length < 5 && (
