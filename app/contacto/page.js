...
export default function Contacto() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const subject = encodeURIComponent(`Contacto desde Argentalk - ${form.nombre}`);
    const body = encodeURIComponent(`Nombre: ${form.nombre}\nEmail: ${form.email}\n\nMensaje:\n${form.mensaje}`);
    window.location.href = `mailto:argentalk26@gmail.com?subject=${subject}&body=${body}`;
    setEnviado(true);
  };
    ...
    <>
      ...
      <div className="container">
        <div className="card">
          ...
          {enviado ? (
            <div className="success" style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 14, color: '#555', marginTop: 4 }}>Se abrió tu app de email con el mensaje preparado</div>
              <button onClick={() => setEnviado(false)} className="btn-secondary" style={{ marginTop: 16 }}>
                Enviar otro mensaje
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label>Mensaje</label>
                <textarea rows={5} placeholder="Contanos en qué podemos ayudarte..." value={form.mensaje} onChange={e => set('mensaje', e.target.value)} required style={{ resize: 'vertical' }} />
              </div>
        ...
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <a href="mailto:argentalk26@gmail.com" style={{ color: '#003DA5', fontWeight: 600, fontSize: 15 }}>
            argentalk26@gmail.com
          </a>
        </div>
      </div>
    </>
