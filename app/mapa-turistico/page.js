...
const DESTINOS = [
  ...
  { id:'elchalten', nombre:'El Chaltén', lat:-49.3314, lng:-72.8868, emoji:'🧗', desc:{es:'La capital del trekking argentino. El Fitz Roy y el Cerro Torre son únicos en el mundo.',en:'Argentina trekking capital. Fitz Roy and Cerro Torre are unique mountain destinations.'}, epoca:{es:'Nov-mar: mejor clima para trekking.',en:'Nov-Mar: best weather for trekking.'}, ciudades:['El Chaltén'] },
  ...
  { id:'tilcara', nombre:'Tilcara', lat:-23.5772, lng:-65.3970, emoji:'🏺', desc:{es:'Pueblo mágico en la Quebrada de Humahuaca, Patrimonio de la Humanidad. Pucará único.',en:'Magical village in the Humahuaca Gorge, UNESCO World Heritage.'}, epoca:{es:'Abr-oct temporada seca. Carnaval en febrero.',en:'Apr-Oct dry season. Carnival in February.'}, ciudades:['Tilcara','Jujuy'] },
  { id:'rosario', nombre:'Rosario', lat:-32.9442, lng:-60.6505, emoji:'🏅', desc:{es:'Cuna de la bandera argentina y de Lionel Messi. Gran movimiento gastronómico y cultural.',en:'Birthplace of the Argentine flag and Lionel Messi. Great gastronomy and culture.'}, epoca:{es:'Todo el año. Primavera y otoño ideales.',en:'Year-round. Spring and autumn ideal.'}, ciudades:['Rosario'] },
    ...
    window.initMapaTuristico = () => {
      ...
      DESTINOS.forEach(d => {
        const marker = new window.google.maps.Marker({
          ...
          map,
          title: d.nombre,
          icon: { url: "data:image/svg+xml;charset=UTF-8," + encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"40\" height=\"40\"><circle cx=\"20\" cy=\"20\" r=\"18\" fill=\"#74ACDF\" stroke=\"white\" stroke-width=\"2\"/><text x=\"20\" y=\"26\" text-anchor=\"middle\" font-size=\"18\">" + d.emoji + "</text></svg>"), scaledSize: new window.google.maps.Size(40, 40) },
        });
        marker.addListener('click', () => {
          const destino = d; setSelected(prev => prev?.id === destino.id ? null : destino);
          markersRef.current.forEach(m => { const de = DESTINOS.find(x => x.id === m.id); if(de) m.marker.setIcon({ url: "data:image/svg+xml;charset=UTF-8," + encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"40\" height=\"40\"><circle cx=\"20\" cy=\"20\" r=\"18\" fill=\"#74ACDF\" stroke=\"white\" stroke-width=\"2\"/><text x=\"20\" y=\"26\" text-anchor=\"middle\" font-size=\"18\">" + de.emoji + "</text></svg>"), scaledSize: new window.google.maps.Size(40, 40) }); });
          marker.setIcon({ url: "data:image/svg+xml;charset=UTF-8," + encodeURIComponent("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"48\" height=\"48\"><circle cx=\"24\" cy=\"24\" r=\"22\" fill=\"#F6B40E\" stroke=\"white\" stroke-width=\"3\"/><text x=\"24\" y=\"31\" text-anchor=\"middle\" font-size=\"22\">" + d.emoji + "</text></svg>"), scaledSize: new window.google.maps.Size(48, 48) });
        });
